...
public class HummingbirdAgent : Agent
{
    ...
    {
        // Convert actions into the array of floats
        ...
        // Calculate new pitch based on smoothed value
        // Clamp pitch to avoid flipping upside down
        float pitch = rotationVector.x + smoothPitchChange * Time.fixedDeltaTime * pitchSpeed;
        if (pitch > 180f) pitch -= 360f;
        pitch = Mathf.Clamp(pitch, -MaxPitchAngle, MaxPitchAngle);

        // Calculate new yaw based on smoothed value
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // Observe a normilized vector poinitng to the nearest flower (3 observations)
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        // All values should be betwen -1 and +1
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        while(!safePositionFound && attemptsRemaining > 0)
        {
            ...
            if (inFrontOfFlower && flowerArea.Flowers.Count > 0)
            {
                ...
                // Point beak at flower (bird's head is center of the transform)
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            if (Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // Note: this is per fixed timestep, meaning it happends .02 second, or 50x per second
